...
async function sendMessageDirect({ sessionId, phone, message, media_path, media_mimetype, media_filename, message_id }) {
    ...
    if (!session?.isReady || !session?.socket) throw new Error(`Session ${sessionId} not ready`);
    ...
    const result = { success: true, sessionId, to: phone, timestamp: Date.now(), message_id: message_id ?? null };
